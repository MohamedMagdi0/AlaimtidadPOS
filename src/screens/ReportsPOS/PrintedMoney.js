...
const CompensatedMoney = ({navigation}) => {
  ...
  const [printedMoneyArray, setPrintedMoneyArray] = React.useState([
    {
      vm_transaction_id: '',
      userName: '',
      userPhoneNumber: '',
  ...
  const [totalPrintedMoney, setTotalPrintedMoney] = React.useState(0);
  ...
  const renderPrintedMoney = ({item}) => {
    console.log(item);
    return (
      <>
        {/* <View style={{flex: 2,flexDirection:'row',justifyContent:'space-around'}}  key = {item.virtual_mony_transaction_id} > */}
        <Card containerStyle={styles.cardCnt} key={item.vm_transaction_id}>
          ...
          <Text style={[styles.cardData, {alignSelf: 'flex-start'}]}>
            الاسم: {item.user_name}
          </Text>
          <Card.Divider />
          <Text style={styles.cardData}>رقم الهاتف: {item.user_phone}</Text>
          <Card.Divider />
          <Text style={styles.cardData}>النوع: {item.user_type}</Text>
          <Card.Divider />
          {/* <Text style={[styles.cardData, {alignSelf:'flex-start'}]} onPress={()=>{copyToClipboard(item.virtual_mony_transaction_pin_number.toString())}}>
                        {item.virtual_mony_transaction_pin_number} :PIN
                      </Text>
                      <Card.Divider/> */}
          ...
          <Card.Divider />
          <Text style={styles.cardData}>
            التاريخ والوقت: {item.vm_transaction_date_time}
          </Text>
        </Card>
        {/* </View> */}
      </>
      ...
      try {
        if (fromDate > toDate) {
          ...
          //console.log("response",response.data.message);
            ...
            setTokenStatus(true);
            setPrintedMoneyArray(response.data.transactions);
            //  setTotalPrintedMoney(response.data.total)
          }
        }
      } catch (e) {
  ...
  } else if (printedMoneyArray.length === 0) {
    return (
      <View style={styles.containerBig}>
        <Text style={styles.nodata}> لا يوجد ارصدة مستلمة مطبوعة</Text>
      </View>
    );
  } else {
    return (
      ...
      showTO
        <Button onPress={showTimepicker} title="Show time picker!" />
                  ...
                  <View style={{flex: 1, padding: 16, marginTop: 12}}>
                    <FlatList
                      data={printedMoneyArray}
                      renderItem={renderPrintedMoney}
...
export default CompensatedMoney;
