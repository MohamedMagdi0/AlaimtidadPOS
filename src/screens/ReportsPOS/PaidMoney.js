...
const Paid = ({navigation}) => {
  ...
  const [paidMoneyArray, setPaidMoneyArray] = React.useState([
    {
      rm_transaction_id: '',
    },
  ]);
  const [totalPaidMoney, setTotalPaidMoney] = React.useState(0);
  ...
  const renderPaidMoney = ({item}) => {
    ...
    return (
      <>
        {/* <View style={{flex: 2,flexDirection:'row',justifyContent:'space-around'}}  key = {item.rm_transaction_id} > */}
        <Card containerStyle={styles.cardCnt} key={item.rm_transaction_id}>
          <View style={{marginTop: hp('1%')}}></View>
          <Text style={styles.cardData}>
            {t('invoicenumber')}: {item.rm_transaction_id}
          </Text>
          <Card.Divider />
          <Text style={styles.cardData}>مسددة الى: {item.toUserName}</Text>
          <Card.Divider />
          <Text style={[styles.cardData, {alignSelf: 'flex-start'}]}>
            رقم الهاتف: {item.toUserPhone}
          </Text>
          <Card.Divider />
          <Text style={[styles.cardData, {alignSelf: 'flex-start'}]}>
            النوع: {item.toUserType}
          </Text>
          <Card.Divider />
          <Text style={styles.cardData}>
            المبلغ المالي الذي تم تسديده:{' '}
            <NumericFormat
          ...
          {/* <Text style={[styles.cardData ,{alignSelf:'flex-start'} ]}>
                                   {item.virtual_mony_serial_number} :Serial
                                   </Text>
                      <Card.Divider/> */}
          <Text style={styles.cardData}>
            {t('thedate')}: {item.creation_date}
          </Text>
          {/* <Card.Divider/>
                      <Text style={styles.cardData}>
    ...
    if (from == '' || to == '') {
      ...
      try {
        if (fromDate > toDate) {
          ...
          const response = await axios.post(
            API_URL +
              `real-money/paid-from-rm?s=${parseInt(
                await AsyncStorage.getItem('userIdInUsers'),
              )}`,
            {
              user_id: userId,
          ...
          if (
            ...
            setTokenStatus(true);
            setPaidMoneyArray(response.data.message);
            setTotalPaidMoney(response.data.total);
          }
        }
      } catch (e) {
            ...
            <View style={styles.containerBig1}>
              <View
                ...
                <SafeAreaView style={{flex: 1, marginTop: hp('5%')}}>
                  {/* <Text style={{fontSize:wp('6%'),color:"#4e31c1",fontFamily:'Cairo-SemiBold',alignSelf:'center',marginBottom:hp('2%')}}>الاموال المسددة</Text>
  ...
  <NumericFormat
      ...
      value={totalPaidMoney} displayType={'text'} thousandSeparator={true} fixedDecimalScale={true} decimalScale={0} />  
    </Text> */}
      ...
      showTO
        <Button onPress={showTimepicker} title="Show time picker!" />
                  ...
                  {paidMoneyArray.length === 0 ? (
                    <View style={styles.containerBig}>
                      <Text style={styles.nodata}> لا يوجد اموال مسددة</Text>
                    </View>
                  ) : (
                    <View style={{flex: 1, padding: 16, marginTop: 12}}>
                      <FlatList
                        data={paidMoneyArray}
                        renderItem={renderPaidMoney}
                        keyExtractor={item => item.rm_transaction_id}
                      />
                    </View>
                  )}
                </SafeAreaView>
