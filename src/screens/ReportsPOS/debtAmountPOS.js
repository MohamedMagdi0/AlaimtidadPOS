...
const DebtAmountPOS = ({navigation}) => {
  ...
  const [debtAmount, setDebtAmount] = React.useState(0);
  ...
  async function getDate() {
    try {
      ...
      //  console.log("response",response.data.message);
        ...
        setDebtAmount(response.data[0].debt_amount);
      }
    } catch (e) {
  ...
  const search = async () => {
    ...
    try {
      const response = await axios.get(
        API_URL +
          `user/eachuserdebtamount?s=${parseInt(
            await AsyncStorage.getItem('userIdInUsers'),
          )}`,
        {
          headers: {
      ...
      //console.log("response",response.data.message);
        ...
        console.log('Else');
        setTokenStatus(true);
        console.log('response', response.data.debtAmountsToUsers);
        setSend(response.data.debtAmountsToUsers);
        setSendlength(response.data.debtAmountsToUsers.length);
      }
    } catch (e) {
      console.log(e.message);
    }
  ...
  //             <Text style={styles.nodata}>لا يوجد اموال تدين بها</Text>
  //         </View>
              ...
              <View
                ...
                <SafeAreaView style={{flex: 1, marginTop: hp('5%')}}>
                  ...
                  {/* <View style={styles.fixToText}>
                                    ...
                                    <View style={styles.fixToText}>
                                        <Text style={{ color: '#4e31c1', fontSize: 20, marginTop: hp('1%'), fontFamily: 'Cairo-SemiBold', }} >{fromDate}</Text>
                                        <Text style={{ color: '#4e31c1', fontSize: 20, marginTop: hp('1%'), fontFamily: 'Cairo-SemiBold', }} >{toDate}</Text>
                                    </View> */}
                      ...
                      {send.map((item, i) => {
                        return (
                          <Card
                            ...
                            key={item.userPhone}>
                            {/* <View style={{ backgroundColor: '#fff', marginTop: hp('-2%'), fontFamily: 'Cairo-SemiBold' }}>
                                                        <Card.Title style={{ color: '#4e31c1', fontFamily: 'Cairo-SemiBold', fontSize: wp('5%'), marginTop: hp('2%') }}> الاموال التي تدين بها</Card.Title>
                                                    </View>
                                                    <Card.Divider /> */}

                            {/* <Text style={{marginBottom: 10,color:'#fff', fontFamily:'Cairo-SemiBold', fontSize:wp('5%')}} key="{cards.sell.message.virtual_mony_transaction_transefer_id}">
                                    {t('transferNumber')}: {item.virtual_mony_transaction_transefer_id} 
                                    </Text>
                                    <Card.Divider/> */}
                            {/* <Text style={{ marginBottom: 10, color: '#4e31c1', fontFamily: 'Cairo-SemiBold', fontSize: wp('5%') }}>
                                                        {t('invoicenumber')}: {item.userPhone}
                                                    </Text>
                                                    <Card.Divider /> */}
                            <Text
                              style={{
                            ...
                            {/* <Card.Divider/>
                                    <Text style={{marginBottom: 10,color:'#4e31c1', fontFamily:'Cairo-SemiBold', fontSize:wp('5%')}}>
                                    {t('categoryType')}: {item.amount_type}
                                    </Text> */}
                            {/* <Card.Divider/>
                                    <Text style={{marginBottom: 10,color:'#fff', fontFamily:'Cairo-SemiBold', fontSize:wp('5%')}}>
                                    {t('extraCost')} : {item.virtual_mony_transaction_extra_fees_id} 
                                    </Text>
            */}
                          </Card>
                        );
