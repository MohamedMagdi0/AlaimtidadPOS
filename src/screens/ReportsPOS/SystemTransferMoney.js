...
const SystemTransferMoney = ({navigation}) => {
  ...
  const [transferedMoneyArray, setTransferedMoneyArray] = React.useState(null);
  const [totalTransferedMoney, setTotalTransferedMoney] = React.useState(0);
  ...
  const renderTransferedMoney = ({item}) => {
    return (
      <>
        <View
          style={{
          ...
          key={item.virtual_mony_transaction_id}>
          <Card
            containerStyle={styles.cardCnt}
            key={item.virtual_mony_transaction_id}>
            <View style={{marginTop: hp('1%')}}></View>
            <Text style={styles.cardData}>
              رقم الكوتة: {item.virtual_money_transaction_bundle_id}
            </Text>
            <Card.Divider />
            {/* <Text style={[styles.cardData,{alignSelf:'flex-start'} ]}>
                      ...
                      <Card.Divider/> */}
            <Text style={styles.cardData}>
              القيمة المحولة: {item.virtual_mony_transaction_value}
            </Text>
            <Card.Divider />
            <Text style={styles.cardData}>تحويل من: {item.from_user_type}</Text>
            <Card.Divider />
            <Text style={styles.cardData}>
              اسم المحول: {item.from_userName}
            </Text>
            <Card.Divider />
            <Card.Divider />
            <Text style={styles.cardData}>
              تاريخ التحويل: {item.creation_date}
            </Text>
          </Card>
        </View>
      </>
  ...
  const search = async () => {
    ...
    if (from == '' || to == '') {
      ...
      try {
        ...
        const response = await axios.post(
          API_URL +
            `specialized-virtual-money-transactions?s=${parseInt(
              await AsyncStorage.getItem('userIdInUsers'),
            )}`,
          {
            to_user_id: parseInt(userId),
            transaction_type: 'transfer',
            category_type: 'system transfer',
            from_date: fromDate,
            to_date: toDate,
          },
          {
        ...
        //console.log("response",response.data.message);
          ...
          setTokenStatus(true);
          setTransferedMoneyArray(response.data.message);
          setTotalTransferedMoney(response.data.total);
        }
      } catch (e) {
  ...
  } else if (transferedMoneyArray && transferedMoneyArray.length == 0) {
    return (
      <View style={styles.containerBig}>
        <Text style={styles.nodata}> لا يوجد ارصدة تحويل النظام</Text>
      </View>
    );
  } else {
    return (
      ...
      showTO
        <Button onPress={showTimepicker} title="Show time picker!" />
                  ...
                  <View style={{flex: 1, padding: 16, marginTop: 12}}>
                    <FlatList
                      data={transferedMoneyArray}
                      renderItem={renderTransferedMoney}
                      keyExtractor={item => item.virtual_mony_transaction_id}
                    />
                  </View>
                </SafeAreaView>
              </View>
...
export default SystemTransferMoney;
