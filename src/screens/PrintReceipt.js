...
class PrintReceipt extends Component {
  ...
  async componentDidMount() {
    ...
    //console.log("paramsssssss222", this.props.route.params);
    if (this.props.route.params.image == undefined) {
      let companyId = this.props.route.params.companyId;
      //console.log("in paramsssssss222 condition", companyId, this.props.route.params.image);
      try {
        ...
        let binary = '';
        let bytes = new Uint8Array(response?.data.printer_logo.data);
        let len = bytes.byteLength;
        for (let i = 0; i < len; i++) {
          binary += String.fromCharCode(bytes[i]);
        }
        //console.log((binary));
        this.setState({coverPrinter: `data:image/png;base64,${binary}`});
      } catch (e) {
        //console.log("errr", e.message);
      }
    }

    if (this.state.soldCards.length <= 5) {
      this.setState({flagShow: true});
      // Alert.alert(
      //     "تمت عملية الشراء بنجاح",
      //     "هل تريد حفظ الكارتات في الاستدوديو",
      //     [
      //       {
      //         text: "Cancel",
      ...
      //         style: "cancel"
      //       },
      //       { text: "OK", onPress: () => this.SaveImageIngallary() }
      //     ]
      //   );
    }
    setTimeout(() => {
  ...
  renderReceipt = ({item, index}) => {
    ...
    //console.log("-------------------------", this.props.route.params.company);
    let itemBarcCodeText = item.pin;
    if (this.props.route.params.company_id == 2) {
      itemBarcCodeText = '*133*' + item.pin + '#';
    } else if (this.props.route.params.company_id == 3) {
      itemBarcCodeText = '*221*' + item.pin + '#';
      //console.log(itemBarcCodeText)
    } else if (this.props.route.params.company_id == 4) {
      itemBarcCodeText = '*101#' + item.pin + '#';
      //console.log(itemBarcCodeText)
    } else if (this.props.route.params.company_id == 41) {
      itemBarcCodeText = '*133*' + item.pin + '#';
      //console.log(itemBarcCodeText)
    } else if (this.props.route.params.company_id == 47) {
      itemBarcCodeText = '*101#' + item.pin + '#';
      //console.log(itemBarcCodeText)
    }

    let itemBarcCodeTextCall = '';
    if (this.props.route.params.company_id == 2) {
      itemBarcCodeTextCall = '*133*PIN# call';
    } else if (this.props.route.params.company_id == 3) {
      itemBarcCodeTextCall = '*221*PIN# call';
      //console.log(itemBarcCodeTextCall)
    } else if (this.props.route.params.company_id == 4) {
      itemBarcCodeTextCall = '*101#PIN# call';
      //console.log(itemBarcCodeTextCall)
    } else if (this.props.route.params.company_id == 41) {
      itemBarcCodeTextCall = '*133*PIN# call';
      //console.log(itemBarcCodeTextCall)
    ...
    //console.log("item.daily_serial", item.daily_serial);
    return (
      <>
        <View style={{backgroundColor: '#ffffff', marginBottom: 20}}>
          ...
          <View style={{backgroundColor: '#fff'}}>
            ...
            {this.props.route.params.soldtoname == '' ? (
              ...
              <>
                <Text
                  ...
                  مباع الى: {this.props.route.params.soldtoname}
                </Text>
                <Text
                  style={{
                    color: '#000',
            ...
            <Text
              style={{color: '#000', fontSize: 0.9 * 23, alignSelf: 'center'}}>
              PIN-CODE
            </Text>
            <Text
              style={{
            ...
            <View>
              ...
              <View
                ...
                <Text
                  ...
                  {' '}
                  {this.props.route.params.selldate}{' '}
                </Text>
                <Text
                  style={{
                    color: '#000',
              ...
              <View
                ...
                <Text
                  ...
                  {this.props.route.params.PosComercialName}{' '}
                </Text>
                <Text
                  style={{
                    color: '#000',
              ...
              {/* <Text style={{color:'#000',fontSize:20755*2 ,marginRight:80,fontWeight:'normal'}}>{'الفئة:'}{this.state.category}</Text>  
                <Text style={{color:'#000',fontSize:20755*2 ,marginRight:80,fontWeight:'normal'}}>{'سعر بيع الكارت:'} {this.state.sellprice}</Text>  */}
              {/* <BarCodeView
                    text={itemBarcCodeText}
              ...
              <View
                ...
                {(console.log(
                  'this.props.show',
                  this.props.route.params.showID,
                ),
                this.props.route.params.showID === 1) ? (
                  <QRCode value={itemBarcCodeText} />
                ) : (
                  <></>
                )}
                <Image
              ...
              <Text
                ...
                {' '}
                {index + 1}/{this.state.soldCards.length}
              </Text>
            </View>
          </View>
        </View>
